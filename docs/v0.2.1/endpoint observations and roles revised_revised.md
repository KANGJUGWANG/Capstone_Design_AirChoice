...
## 1. 문서 목적
본 문서는 Travelpayouts 가격 관련 엔드포인트를 실제 호출하여 확인한 관측 결과를 정리하고, 현재 프로젝트에서 각 엔드포인트를 어떤 역할로 사용할지 정리하기 위해 작성한다.
...
- 비교 테스트에서 row 수는 확보되었으나, 현재 프로젝트 기준에서는 `prices_for_dates`와 `get_latest_prices` 사이에서 역할이 애매함
...
## 5. 엔드포인트 역할 잠정 확정
> 아래 역할 정의는 현재 프로젝트 운영안이다.  
> 이후 실제 누적 테스트에서 누락률, 중복 패턴, 행 수 분포, 응답 안정성, 공통 스키마 매핑 적합성이 달라질 경우 재조정할 수 있다.
...
- 현재 프로젝트의 공통 스키마 초안으로 가장 자연스럽게 매핑 가능
...
### 6-3. raw 응답 보존 필요
- 엔드포인트마다 필드 구조 차이가 있으므로 원본 응답(raw_json)은 반드시 저장하는 것을 기본 원칙으로 한다

### 6-4. 정규화 스키마는 후속 검증 필요
- 현재 정리 중인 공통 스키마는 **초안/가정** 수준으로 본다
- 실제 누적 응답 확보 후 누락 필드, 불필요 필드, 중복 의미 필드 여부를 다시 검토한다
- 최종 학습 입력 스키마는 현재 단계에서 확정하지 않는다
...
- 해당 기준은 대상 항공사 공식 예약 가능 기간 및 보조 근거를 반영한 **현재 운영 기준**이다.

### 7-2. 수집 기간 분할
- 장기 수집 시 `limit` / `page` 누락 가능성이 있으므로 한 번에 긴 기간을 요청하기보다 **분할 수집 방식**을 우선 검토한다.
...
## 8. 현재 해석
- `prices_for_dates`는 **메인 누적용 후보**
- `get_latest_prices`는 **보조 분포 보강용 후보**
- `cheap`은 **기준점/검증용**
- `month-matrix`는 **현재 보류**

이 기준은 현재 프로젝트 운영 기준이며, 향후 테스트 결과 누락/중복/행 수 분포가 달라질 경우 문서를 업데이트한다.

---

## 9. 다음 단계
- `prices_for_dates` 기준 공통 스키마 초안 정교화
- `get_latest_prices`를 같은 스키마에 보조 테이블로 매핑 가능 여부 검토
- 원본 보존(raw) / 중간 정규화(interim) / 후속 분석용(processed) 구분 정리
